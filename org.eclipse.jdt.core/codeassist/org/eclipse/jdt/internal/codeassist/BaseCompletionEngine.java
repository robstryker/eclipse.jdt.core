...
public abstract class BaseCompletionEngine
	extends Engine
	implements ICompletionEngine, ISearchRequestor, TypeConstants , TerminalTokens , RelevanceConstants, SuffixConstants {
	...
	public class CompletionProblemFactory extends DefaultProblemFactory {
		...
		private CategorizedProblem checkProblem(CategorizedProblem pb,
			...
			int id = pb.getID();
			if (BaseCompletionEngine.this.actualCompletionPosition > start
				&& this.lastErrorStart < start
				&& pb.isError()
				&& (id & IProblem.Syntax) == 0
				&& (BaseCompletionEngine.this.fileName == null || CharOperation.equals(BaseCompletionEngine.this.fileName, originatingFileName))) {

				BaseCompletionEngine.this.problem = pb;
				this.lastErrorStart = start;
			}
				...
				switch (id) {
					case IProblem.UsingDeprecatedType:
						this.hasForbiddenProblems =
							BaseCompletionEngine.this.options.checkDeprecation;
						break;
					case IProblem.NotVisibleType:
						this.hasForbiddenProblems =
							BaseCompletionEngine.this.options.checkVisibility;
						break;
					case IProblem.ForbiddenReference:
						this.hasForbiddenProblems =
							BaseCompletionEngine.this.options.checkForbiddenReference;
						break;
					case IProblem.DiscouragedReference:
						this.hasForbiddenProblems =
							BaseCompletionEngine.this.options.checkDiscouragedReference;
						break;
					default:
	...
	*/
	...
	public BaseCompletionEngine(
		...
		this.parser =
				createCompletionParser(this.problemReporter, this.requestor, monitor);
		this.owner = owner;
		this.monitor = monitor;
	}

	protected abstract ICompletionParserFacade createCompletionParser(ProblemReporter problemReporter2, CompletionRequestor requestor2, IProgressMonitor monitor2);
	...
	public void acceptUnresolvedName(char[] name) {
		...
		BaseCompletionEngine.this.noProposal = false;
		if(!BaseCompletionEngine.this.requestor.isIgnored(CompletionProposal.LOCAL_VARIABLE_REF)) {
			InternalCompletionProposal proposal = BaseCompletionEngine.this.createProposal(CompletionProposal.LOCAL_VARIABLE_REF, BaseCompletionEngine.this.actualCompletionPosition);
			...
			proposal.setTokenRange(this.tokenStart - this.offset, this.tokenEnd - this.offset);
			proposal.setRelevance(relevance);
			BaseCompletionEngine.this.requestor.accept(proposal);
			if(DEBUG) {
				BaseCompletionEngine.this.printDebug(proposal);
			}
		}
	}
	...
	private void buildContext(
		...
		buildContextSetTokenRange(context, astNode);
	...
	protected abstract void buildContextSetTokenRange(InternalCompletionContext context, ASTNode astNode);
	...
	public void complete(ICompilationUnit sourceUnit, int completionPosition, int pos, ITypeRoot root) {
		...
		try {
			...
			CompilationUnitDeclaration parsedUnit = this.parser.parseCompilationUnitDeclaration(sourceUnit, result, this.actualCompletionPosition);
				...
				if (parsedUnit.isModuleInfo()) {
					...
					try {
						...
						for (int i = 0, l = providesStmts != null ? providesStmts.length : 0; i < l; ++i) {
							...
							this.parser.setEnclosingNode(providesStmt);
						...
						if (e.astNode != null) {
							...
							if(DEBUG) {
								...
								ASTNode node = this.parser.getAssistNodeParent();
								if(node != null) {
									trace("COMPLETION - Parent Node : " + this.parser.getAssistNodeParent()); //$NON-NLS-1$
								}
							}
	...
	private boolean completeJavadocTagInModuleInfo(CompilationUnitDeclaration parsedUnit) {
		boolean contextAccepted = false;
		if (this.parser.getAssistNodeParent() instanceof CompletionJavadoc && parsedUnit.isModuleInfo() ) {
			try {
				this.lookupEnvironment.buildTypeBindings(parsedUnit, null /*no access restriction*/);
				if(this.parser.getAssistNode() instanceof CompletionOnJavadocTag) {
					((CompletionOnJavadocTag)this.parser.getAssistNode()).filterPossibleTags(parsedUnit.scope);
				}
				throw new CompletionNodeFound(this.parser.getAssistNode(), null, parsedUnit.scope);
			}
			catch (CompletionNodeFound e) {
	...
	public void complete(IType type, char[] snippet, int position, char[][] localVariableTypeNames, char[][] localVariableNames, int[] localVariableModifiers, boolean isStatic){
		...
		try {
			...
			if (type instanceof SourceType) {
				...
				// questionable edit: this.parser.problemReporter to this.problemReporter; should be same value?
				typeDeclaration = new BinaryTypeConverter(this.problemReporter, compilationResult, null/*no need to remember type names*/).buildTypeDeclaration(type, compilationUnit);
			}
	...
	private void completionOnProvidesImplementationsQualifiedTypeReference(ASTNode astNode, ASTNode astNodeParent, Binding qualifiedBinding, Scope scope) {
		findImplementations((ProvidesStatement) this.parser.getEnclosingNode(), (TypeReference) astNode);
	}
	...
	private void completionOnSingleTypeReference(ASTNode astNode, ASTNode astNodeParent, Binding qualifiedBinding, Scope scope) {
		...
		ASTNode parentNode = this.parser.getAssistNodeParent();
	...
	private void computeExpectedTypes(ASTNode parent, ASTNode node, Scope scope){
		...
		if(parent instanceof AbstractVariableDeclaration && !(parent instanceof TypeParameter)) {
			...
			if (this.parser.getEnclosingNode() instanceof AbstractVariableDeclaration ||
					this.parser.getEnclosingNode() instanceof ReturnStatement) {
				// completing inside the diamond
				if (this.parser.getEnclosingNode() instanceof AbstractVariableDeclaration) {
					AbstractVariableDeclaration abstractVariableDeclaration = (AbstractVariableDeclaration) this.parser.getEnclosingNode();
					expected = abstractVariableDeclaration.initialization != null ? abstractVariableDeclaration.initialization.expectedType() : null;
				} else {
					ReturnStatement returnStatement = (ReturnStatement) this.parser.getEnclosingNode();
...
//		}
	...
	private int computeRelevanceForSuper(MethodBinding method, Scope scope, InvocationSite site) {
		if (site instanceof CompletionOnMemberAccess) {
			...
			if (access.isSuperAccess() && this.parser.getAssistNodeParent() == null) {
				ReferenceContext referenceContext = scope.referenceContext();
	...
	private void findConstructorsFromMissingType(
		...
		IMissingTypesGuesser missingTypesConverter = createMissingTypeGuesser();
	...
	private void findFields(
		...
		for (int f = fields.length; --f >=0;) {
			...
			ASTNode astNode = this.parser.getAssistNode();
		...
		MissingTypesGuesser.GuessedTypeRequestor substitutionRequestor =
			new MissingTypesGuesser.GuessedTypeRequestor() {
				...
				public void accept(
						...
						boolean hasProblems) {
					findFieldsAndMethods(
						BaseCompletionEngine.this.completionToken,
						guessedType,
						scope,
						new ObjectVector(),
						new ObjectVector(),
...
//				boolean inlineTagStarted = javadocTag.completeInlineTagStarted();
	...
	private int computeRelevanceForFinalInIOE() {
		...
		return this.parser.getAssistNodeParent() instanceof InstanceOfExpression ? R_FINAL + R_EXPECTED_TYPE : 0;
	}
	...
	private char[] findParameterNameAtLocationFromAssistParent(ObjectVector methodsFound, Scope scope) {
		...
		if (this.parser.getAssistNodeParent() instanceof MessageSend ms) {
		...
		} else if (this.parser.getAssistNode() instanceof MessageSend ms) {
			arguments = ms.arguments;
		...
		} else if (this.parser.getAssistNodeParent() instanceof AllocationExpression ae) {
		...
		for (MethodBinding binding : candidates) {
			if (binding.parameterNames.length >= argumentLength) {
				for (int i = 0; i < argumentLength; i++) {
					if (arguments == null || arguments[i] == this.parser.getAssistNode()) {
	...
	protected abstract IMissingTypesGuesser createMissingTypeGuesser();

	protected abstract IUnresolvedReferenceNameFinder createUnresolvedReferenceNameFinder();
		...
		UnresolvedReferenceNameFinder.UnresolvedReferenceNameRequestor nameRequestor =
			new UnresolvedReferenceNameFinder.UnresolvedReferenceNameRequestor() {
				...
				public void acceptName(char[] name) {
					BaseCompletionEngine.this.acceptUnresolvedName(name);
					proposedNames.add(name);
				}
			};

		ReferenceContext referenceContext = scope.referenceContext();
		IUnresolvedReferenceNameFinder nameFinder = createUnresolvedReferenceNameFinder();
		if( nameFinder == null )
			return null;
		if (referenceContext instanceof AbstractMethodDeclaration) {
			...
			UnresolvedReferenceNameFinder.UnresolvedReferenceNameRequestor nameRequestor =
				new UnresolvedReferenceNameFinder.UnresolvedReferenceNameRequestor() {
					...
					public void acceptName(char[] name) {
						int relevance = computeBaseRelevance();
						relevance += computeRelevanceForInterestingProposal();
						relevance += computeRelevanceForCaseMatching(BaseCompletionEngine.this.completionToken, name);
						...
						// accept result
						BaseCompletionEngine.this.noProposal = false;
						if(!BaseCompletionEngine.this.requestor.isIgnored(CompletionProposal.VARIABLE_DECLARATION)) {
							InternalCompletionProposal proposal =  BaseCompletionEngine.this.createProposal(CompletionProposal.VARIABLE_DECLARATION, BaseCompletionEngine.this.actualCompletionPosition);
							...
							proposal.setCompletion(name);
							//proposal.setFlags(Flags.AccDefault);
							proposal.setReplaceRange(BaseCompletionEngine.this.startPosition - BaseCompletionEngine.this.offset, BaseCompletionEngine.this.endPosition - BaseCompletionEngine.this.offset);
							proposal.setTokenRange(BaseCompletionEngine.this.tokenStart - BaseCompletionEngine.this.offset, BaseCompletionEngine.this.tokenEnd - BaseCompletionEngine.this.offset);
							proposal.setRelevance(relevance);
							BaseCompletionEngine.this.requestor.accept(proposal);
							if(DEBUG) {
								BaseCompletionEngine.this.printDebug(proposal);
	...
	public AssistParser getParser() {
		if( this.parser instanceof AssistParser ) {
			return ((AssistParser) this.parser);
		}
		return null;
	}
	...
	private Initializer parseSnippeInitializer(char[] snippet, int position, char[][] localVariableTypeNames, char[][] localVariableNames, int[] localVariableModifiers, boolean isStatic){
		...
		CompilationUnitDeclaration fakeAST = this.parser.parseCompilationUnitDeclaration(fakeUnit, fakeResult, this.actualCompletionPosition);
